...
import { sessionFingerprint } from './fingerprint.mjs'
...
/* — Markdown 渲染（正文走 renderMarkdown；reasoning/工具结果仍是 pre 纯文本）— */
.msg-body h1, .msg-body h2, .msg-body h3, .msg-body h4, .msg-body h5, .msg-body h6 { margin: 12px 0 6px; line-height: 1.35; }
.msg-body h1 { font-size: 1.3em; } .msg-body h2 { font-size: 1.2em; } .msg-body h3 { font-size: 1.1em; }
.msg-body h4, .msg-body h5, .msg-body h6 { font-size: 1em; }
.msg-body > :first-child, .msg-body h1:first-child, .msg-body h2:first-child, .msg-body h3:first-child { margin-top: 0; }
.msg-body ul, .msg-body ol { margin: 4px 0 8px; padding-left: 22px; }
.msg-body li { margin: 2px 0; }
.msg-body .md-code { background: var(--accent-bg); border-radius: 4px; padding: 1px 5px; font-family: ui-monospace, SFMono-Regular, Consolas, "Courier New", monospace; font-size: .92em; }
.msg-user .md-code { background: rgba(255,255,255,.18); }
.msg-body pre.md-pre { margin: 6px 0 10px; padding: 10px 12px; background: var(--bg2); border: 1px solid var(--border); border-radius: 8px; overflow-x: auto; }
.msg-body pre.md-pre code { background: none; padding: 0; font-family: ui-monospace, SFMono-Regular, Consolas, "Courier New", monospace; font-size: 12px; line-height: 1.5; white-space: pre-wrap; word-break: break-word; }
.msg-body blockquote.md-quote { margin: 6px 0 10px; padding: 2px 12px; border-left: 3px solid var(--border); color: var(--fg-dim); }
.msg-body table.md-table { border-collapse: collapse; margin: 6px 0 10px; font-size: .95em; display: block; overflow-x: auto; max-width: 100%; }
.msg-body table.md-table th, .msg-body table.md-table td { border: 1px solid var(--border); padding: 4px 10px; text-align: left; }
.msg-body table.md-table th { background: var(--bg2); }
.msg-body a { color: var(--accent); }
.msg-user a { color: var(--user-fg); text-decoration: underline; }
.msg-body hr.md-hr { border: none; border-top: 1px solid var(--border); margin: 12px 0; }
...
export function renderSessionPage(events, options = {}) {
  ...
  const fingerprint = sessionFingerprint(events)
  ...
  <div class="msg-text msg-body">${renderMarkdown(m.text)}</div>
</article>`
    }
    const parts = []
  ...
  <div class="msg-meta"><span class="role-badge">Assistant</span>${model ? `<span class="model-badge">${esc(model)}</span>` : ''}<time>${esc(fmtTime(m.time))}</time></div>`)
    if (m.text) parts.push(`  <div class="msg-text msg-body">${renderMarkdown(m.text)}</div>`)
