...
| 社区 `dsh-chat-import`、`dsh-session-share` | 机器间数据迁移 | 机器 |
| **本插件 `dsh-share-page`** | **可读的分享网页** | **人** |

**面向人的可读分享网页此前是空白** —— 本插件补上这一环：一份双击即开、离线可用、被改动就能发现的会话快照。
...
# 2) 克隆本仓库
git clone https://github.com/<你的用户名>/dsh-share-page.git
cd dsh-share-page

# 3) 以本地路径安装到 web profile（开发/试用）
dsh plugin --profile web add "file:$(pwd)"

# 4) 重启 dsh，会话 Header 右侧即出现「分享」按钮
dsh --profile web
```

打开任意会话 → 点 **分享** → 选好选项 → 生成。完整用法见下方[使用方式](#使用方式)。
...
> 路径写法：`file:` 后接仓库目录。克隆后通常就是 `file:$(pwd)`（当前目录）。
